...
<?php if (!$comentario->bloqueado || Yii::$app->user->can('admin')): ?>
	<div class="comentario" style="margin-left: <?= $nivel * 20 ?>px; border-left: <?= $nivel > 0 ? '2px solid #ccc' : 'none' ?>; padding-left: 10px;">
		<p>
			...
			(<?= Yii::$app->formatter->asDatetime($comentario->fecha_creacion) ?>)
			<?php if ($comentario->bloqueado): ?>
				<span class="text-danger">[Comentario bloqueado]</span>
			<?php endif; ?>
		</p>
		<p><?= Html::encode($comentario->texto) ?></p>
		
		<?php if (!$comentario->cerrado && !$comentario->bloqueado): ?>
			<?= Html::a('Responder', ['comentario/create', 'oferta_id' => $comentario->oferta_id, 'comentario_origen_id' => $comentario->id], ['class' => 'btn btn-link']) ?>
		<?php endif; ?>
		
		<?php if (!$comentario->cerrado && !$comentario->bloqueado && Yii::$app->user->id !== $comentario->usuario_id): ?>
			<?= Html::a('Denunciar', '#', [
